...
def adjust_for_overlap(inst, idx, duration, start_time, notes_to_remove, notes_to_add):
    ...
    if idx > 0:
        ...
        if start_time < previous_note.end:
            ...
            for note in notes_to_remove:
                if note.pitch == previous_note.pitch and note.start == previous_note.start and note.end == previous_note.end:
            ...
            if previous_note.end <= previous_note.start:
                print(f"duration3: {previous_note.end - previous_note.start}", flush=True)
    if idx < len(inst.notes) - 1:
        ...
        if start_time + duration > next_note.start:
            ...
            # TODO: this is problematic because the next note could be very far away?
            # allow overlap is true so this should not be a problem
...
def gather_pitches_and_adjust_pitch(inst, idx, stdev_pitch_delta, rng):
    ...
    while next_note_index < len(inst.notes) - 1  and inst.notes[next_note_index].start == note.start:
        print("while 1")
    ...
    if next_note_index < len(inst.notes) - 1:
        ...
        while next_next_note_index < len(inst.notes) - 1 and inst.notes[next_next_note_index].start == next_note.start:
            print("while 2")
            next_note_pitches.append(inst.notes[next_next_note_index].pitch)
            next_next_note_index += 1
    
    # Adjust the pitch using a normal distribution until a pitch + delta is found in next_note_pitches
    pitch_delta = 0
    rng = np.random.default_rng()
...
def add_screwups(
    ...
    for inst_index, inst in enumerate(midi.instruments):
        ...
        for idx in error_indices:
            ...
            if screwup_type == 1:  # Plays wrong note
                ...
                if allow_overlap is False:
                    ...
                    pitch_delta = gather_pitches_and_adjust_pitch(inst, idx, stdev_pitch_delta, rng)
                initial_note_dur = note_duration / 10.0 + rng.uniform(
                    low=-1 * note_duration / 50.0, high=note_duration / 50.0
                )
        ...
        notes_to_remove_set = set((note.pitch, note.start, note.end) for note in notes_to_remove)

        # Then filter the notes
        inst.notes = [note for note in inst.notes if (note.pitch, note.start, note.end) not in notes_to_remove_set]
        for note in notes_to_remove:
...
def get_extra_notes(
    ...
    next_note_idx = idx + 1
    next_note_start = inst.notes[next_note_idx].start if next_note_idx < len(inst.notes) else note.end
    
    if not allow_overlap:
        print(f"Adjusting for overlap for note {idx}")
        while next_note_start <= note.start:
            next_note_start = inst.notes[next_note_idx + 1].start if next_note_idx < len(inst.notes) - 1 else note.end        
...
def add_timing_inaccuracies(
    ...
    timing_shift_seconds = rng.normal(
                loc=config["expressive_timing_mean_ms"] / 1000.0, # we use zero for equal chance of early and late
    ...
    if not allow_overlap:
        ...
        if idx < len(inst.notes) - 1 and idx > 0:
            next_note = inst.notes[idx + 1]
            previous_note = inst.notes[idx - 1]
            while (timing_shift_seconds > (next_note.end - note.start)) or (timing_shift_seconds < (previous_note.start - note.start)):
                timing_shift_seconds = rng.normal(
                    loc=config["expressive_timing_mean_ms"] / 1000.0, # we use zero for equal chance of early and late
                    scale=config["expressive_timing_std_ms"] / 1000.0,
                )
            ...
            next_note = inst.notes[idx + 1]
            while timing_shift_seconds > (next_note.end - note.start):
                timing_shift_seconds = rng.normal(
                    loc=config["expressive_timing_mean_ms"] / 1000.0, # we use zero for equal chance of early and late
                    scale=config["expressive_timing_std_ms"] / 1000.0,
                )
        ...
        if idx > 0:
            ...
            if note.start < previous_note.end:
                ...
                for n in notes_to_remove:
                    if n.pitch == previous_note.pitch and n.start == previous_note.start and n.end == previous_note.end:
                ...
                if previous_note.end <= previous_note.start:
                    print(f"duration1: {previous_note.end - previous_note.start}", flush=True)

        if idx < len(inst.notes) - 1:
            next_note = inst.notes[idx + 1]
            if note.end > next_note.start:
                if note.start >= next_note.start:
                    # if note.start is greater than next_note.start, remove the note
                    print(f"note.start: {note.start}, next_note.start: {next_note.start}")
                    inst.notes.remove(note)
                    notes_to_remove.append(note)
                else:
                    # Adjust current note to avoid overlap
